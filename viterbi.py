...
def tag():
    __fTrain = codecs.open("F:\\College\\hindi-pos-tagger-master\\hindi-pos-tagger-master\\Viterbi and SVM\\new_train.txt", mode="r", encoding="utf-8")
    __fTest  = codecs.open("F:\\College\\hindi-pos-tagger-master\\hindi-pos-tagger-master\\Viterbi and SVM\\tagfile.txt", mode="r", encoding="utf-8")
    ...
    D = {v: k for k, v in OrderedDict(enumerate(all_tags)).items()}
    ...
    for line in __fTest.readlines():
        try:
            ...
            final_output.append(tag_seq)
            #print(tag_seq)
        except:
            #print("Couldn't tag sentence ...")
            final_output.append(["Couldn't tag sentence ..."])
            continue
    return(final_output)

def test():
    __fTrain = codecs.open("F:\\College\\hindi-pos-tagger-master\\hindi-pos-tagger-master\\Viterbi and SVM\\new_train.txt", mode="r", encoding="utf-8")
    __fTest  = codecs.open("F:\\College\\hindi-pos-tagger-master\\hindi-pos-tagger-master\\Viterbi and SVM\\new_test.txt", mode="r", encoding="utf-8")
